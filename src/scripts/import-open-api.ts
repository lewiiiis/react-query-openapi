  ...
  if (!skipReact) {
    ...
    ${paramsTypes ? `${paramsTypes};\n\t` : ""} ${queryParamsType ? `params?: ${componentName}QueryParams;\n\t` : ""} ${
      ...
      output += `export const use${componentName} = (mutationOptions?: MutationOptions) => useMutation<${responseType}, any, Use${componentName}Variables>(${path.replace(
        ...
        queryParamsType ? ",{params}" : ",{}"
      }).then(data => data?.data), mutationOptions);\n\n`;
      ...
      } ${queryParamsType ? ",{params}" : ",{}"}).then(data => data?.data)`;
    }
  }
 ...
 * @param version
 */
export const addCommonComponentsAndHooks = () => `
...
const importOpenApi = async ({
  ...
  let outputHeaders = "/* Generated by react-query-openapi */\n\n";
