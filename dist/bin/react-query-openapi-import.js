...
var generateRestfulComponent = function (operation, verb, route, operationIds, parameters, schemasComponents, skipReact, pathParametersEncodingMode, customGenerator) {
    ...
    if (!skipReact) {
        ...
        if (verb === "get") {
            ...
            output += "export const use" + componentName + " = (mutationOptions?: MutationOptions) => useMutation<" + responseType + ", any, Use" + componentName + "Variables>(" + path.replace(/[{}$]/g, "") + ", ({" + (paramsInPath.length > 0 ? paramsInPath.join(", ") + "," : "") + " " + (queryParamsType ? "params," : "") + " " + (needARequestBodyComponent ? "body," : "") + " }) => axios." + verb + "(" + path + " " + (needARequestBodyComponent ? ",body" : verb !== "delete" ? ",{}" : "") + " " + (queryParamsType ? ",{params}" : ",{}") + ").then(data => data?.data ?? data), mutationOptions);\n\n";
            output += "export const api" + componentName + " = ({" + (paramsInPath.length > 0 ? paramsInPath.join(", ") + "," : "") + " " + (queryParamsType ? "params," : "") + " " + (needARequestBodyComponent ? "body," : "") + " }: Use" + componentName + "Variables) => axios." + verb + "<" + responseType + ">(" + path + " " + (needARequestBodyComponent ? ",body" : verb !== "delete" ? ",{}" : "") + " " + (queryParamsType ? ",{params}" : ",{}") + ").then(data => data?.data ?? data)";
        }
    }
    // Custom version
    if (customGenerator) {
