    ...
    if (!skipReact) {
        ...
        if (verb === "get") {
            ...
            output += "export const use" + componentName + " = (" + (paramsInPath.length || queryParamsType
                ...
                .join(", ") + "}") + ") => axios." + verb + "(" + path.replace(/\${/g, "${_") + " " + (queryParamsType ? ",{params: _params}" : "") + "), { refetchOnMount: false, ...queryOptions });\n\n";
            output += "export const useInvalidate" + componentName + " = (" + paramsTypes + ") => useInvalidateQuery(" + path + ", \"invalidate" + componentName + "\");\n\n";
            output += "export const api" + componentName + " = (" + ("{" + (queryParamsType ? "params," : "") + " " + paramsInPath.join(", ") + "}: Omit<Use" + componentName + "Props, \"queryOptions\">") + ") => axios." + verb + "<" + responseType + ">(" + path + " " + (queryParamsType ? ",{params}" : "") + ")";
        }
        else {
            output += "export interface Use" + componentName + "Variables {\n        " + (paramsTypes ? paramsTypes + ";\n\t" : "") + " " + (queryParamsType ? "params: " + componentName + "QueryParams;\n\t" : "") + " " + (needARequestBodyComponent ? "body: " + componentName + "RequestBody;\n\t" : "") + "\n        } \n\n";
            output += "export const use" + componentName + " = (mutationOptions?: MutationOptions) => useMutation<" + responseType + ", any, Use" + componentName + "Variables>(" + path.replace(/[{}$]/g, "") + ", ({" + (paramsInPath.length > 0 ? paramsInPath.join(", ") + "," : "") + " " + (queryParamsType ? "params," : "") + " " + (needARequestBodyComponent ? "body," : "") + " }) => axios." + verb + "(" + path + " " + (needARequestBodyComponent ? ",body" : verb !== "delete" ? ",{}" : "") + " " + (queryParamsType ? ",{params}" : ",{}") + "), mutationOptions);\n\n";
            output += "export const api" + componentName + " = ({" + (paramsInPath.length > 0 ? paramsInPath.join(", ") + "," : "") + " " + (queryParamsType ? "params," : "") + " " + (needARequestBodyComponent ? "body," : "") + " }: Use" + componentName + "Variables) => axios." + verb + "<" + responseType + ">(" + path + " " + (needARequestBodyComponent ? ",body" : verb !== "delete" ? ",{}" : "") + " " + (queryParamsType ? ",{params}" : ",{}") + ")";
        }
    }
    // Custom version
    if (customGenerator) {
