    ...
    if (!skipReact) {
        ...
        if (verb === "get") {
            output += "export const use" + componentName + " = (" + (paramsInPath.length || queryParamsType
                ...
                : "{queryOptions}") + ": Use" + componentName + "Props) => useQuery<" + responseType + ">(" + path + (queryParamsType ? " + " + "`?${Object.keys(params).map(key => `${key}=${params[key]}`).join('&')}`" : "") + ", () => axios." + verb + "(" + path + " " + (queryParamsType ? ",{params}" : "") + ").then(data => data.data), queryOptions);\n\n";
