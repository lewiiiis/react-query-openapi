...
var generateRestfulComponent = function (operation, verb, route, operationIds, parameters, schemasComponents, skipReact, pathParametersEncodingMode, customGenerator) {
    ...
    if (!skipReact) {
        ...
        if (verb === "get") {
            ...
            output += "export const api" + componentName + " = ({" + (paramsInPath.length === 1 ? "" + paramsInPath : paramsInPath.join(", ")) + " " + (needARequestBodyComponent ? (paramsInPath.length ? "," : "") + " body" : "") + " }: Use" + componentName + "Variables) => axios." + verb + "<" + responseType + ">(" + path + " " + (needARequestBodyComponent ? ",body" : verb !== "delete" ? ",{}" : "") + " " + (queryParamsType ? ",{params}" : ",{}") + ").then(data => data?.data ?? data)";
        }
    }
    // Custom version
    if (customGenerator) {
